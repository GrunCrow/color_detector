...
while (True):
    ...
    text = color_rec + ' R=' + str(r) + ' G=' + str(g) + ' B=' + str(b)
    ...
    if played is False:
        played = True
        playsound('{}.mp3'.format(color_rec))

    
    if clicked is True:
        clicked = False
        played = False
        if os.path.exists('{}.mp3'.format(color_rec)):
            os.remove('{}.mp3'.format(color_rec))
        ...
        color_rec = recognize_color(r, g, b)
        file_name = color_rec + '.mp3'
        var = gTTS(color_rec, lang = 'en')
        var.save(file_name)
